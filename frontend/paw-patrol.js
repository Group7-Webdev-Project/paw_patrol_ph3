...
// Parallax for `.wrapper` (background + foreground images)
(function(){
    ...
    function onScroll() {
        ...
        window.requestAnimationFrame(() => {
            ...
            // amount the wrapper has moved relative to the viewport
            const move = -rect.top;
            // background moves slower, foreground moves faster for depth
            if (bg) bg.style.transform = `translate3d(-50%, ${move * 0.25}px, 0)`;
            if (fg) fg.style.transform = `translate3d(-50%, ${move * 0.35}px, 0)`;
            ticking = false;
        });
    }
